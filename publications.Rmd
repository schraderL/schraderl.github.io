...
paperTab <- function(author.number) {
  ...
  kbl(paperTable, caption = paste(nrow(paperTable)," publications (co-)authored by", auth.profile[[author.number]]$name," ([google scholar](https://scholar.google.de/citations?user=",scholar_id[author.number],"))",sep=""),row.names = F) %>% kable_styling(bootstrap_options = c("striped", "hover"))
}
